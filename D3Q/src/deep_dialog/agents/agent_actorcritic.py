...
from deep_dialog.a2c import A2C
...
class AgentA2C(Agent):
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, params=None):
        ...
        self.experience_replay_pool_size = params.get('experience_replay_pool_size', 1000)  # 10000
        ...
        self.a2c = A2C(self.state_dimension, self.hidden_size, self.num_actions)  # (213, 80, 31)
    ...
    def state_to_action(self, state):  # （该函数在DM模块中被调用）
    ...
    # epsilon-贪婪策略-->使用Actor网络
    def run_policy(self, representation):
        # 若启用热启动，且回放缓存池未满，则使用规则策略。
        if self.warm_start == 1:
            if len(self.experience_replay_pool) > self.experience_replay_pool_size:
                self.warm_start = 2
            return self.rule_policy()  # 返回的是索引

        # 若不是热启动阶段，则基于Actor选择动作
        else:
            a = self.a2c.predict_a(representation)
            return a
    ...
    def action_index(self, act_slot_response):
        ...
        for (i, action) in enumerate(self.feasible_actions):  # 在self.feasible_actions中查找动作
            if act_slot_response == action:  # 若动作吻合
                return i  # 则返回对应的索引, 概率为1
    ...
    def load_trained_a2c(self, path):
    ...
    def save_a2c(self, path):
        # return self.dqn.unzip()
        self.a2c.save_model(path + ".actor.ckpt")

    def load_a2c(self, params):
        self.a2c.load(params)
