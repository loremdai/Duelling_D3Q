...
class ModelBasedSimulator(UserSimulator):
    ...
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, start_set=None, params=None):
        ...
        self.model = ModelBasedUsersimulator(
            2 * (2 * self.slot_cardinality + self.act_cardinality) + 2 * self.slot_cardinality, 80,
            self.act_cardinality, 2 * self.slot_cardinality)
        ...
        self.training_goal_ids = json.load(
            open(u'D:\code\DialoguePlaning\TC-Bot_model_based_simulator\src\deep_dialog\data\\user_goals_ids.json'))
    ...
    def prepare_state_representation_per_step(self, message):
        ...
        for m_idx, m in enumerate(message):
            ...
            if actor == 'agent' and m_idx != len(message) - 1:
                message_representation = np.hstack(
                    [message_representation, self.last_user_representation, self.user_goal_representation])
